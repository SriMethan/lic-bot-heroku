...
def play_game(li, game_id, control_queue, engine_factory, user_profile, config, challenge_queue, correspondence_queue, logging_queue, logging_configurer, logging_level):
    ...
    conversation.send_reply(SendLine('player'), f'Hi My owner is @drdisrespect1 ,All The Best @{opponent}')
    conversation.send_reply(SendLine('spectator'), f'Hello! Welcome my game,Stay tuned @{opponent}')
    ...
    conversation.send_reply(SendLine('player'), f'Nice Game, Thaks for your game!')
    conversation.send_reply(SendLine('spectator'), f'Is it Nice Game, Thanks for watching my game!')
